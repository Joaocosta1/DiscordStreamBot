...
Screen sharing uses Discord's browser Go Live. Chrome is launched with: `--use-fake-device-for-media-stream` (fake mic/camera — Discord needs an audio input device to connect to voice, else it shows error **3002**); `--disable-background-timer-throttling` / `--disable-backgrounding-occluded-windows` / `--disable-renderer-backgrounding` (keep the YouTube tab playing while the voice tab is foregrounded for sharing — without these YouTube shows "Algo deu errado" and playback/audio stalls); `--disable-accelerated-video-encode` (software encode). **No `--mute-audio`** — the tab must produce audio so the Go Live stream carries the video's sound (the bot's PC also plays it locally). After joining, `EnsureSelfDeafenedAsync` clicks Discord's **deafen** control (mutes mic + incoming audio; deafen does not stop the outgoing Go Live audio); it only clicks when the button's `aria-label` indicates the "activate" state, so a persisted deafened session isn't toggled off. `TryStartScreenShareAsync` probes several pt-BR share-button labels and `JoinVoiceAndShareAsync` retries up to `ShareMaxAttempts` (3); on final failure `LogVoicePageStateAsync` dumps the visible voice-panel button labels for diagnosis.

After the share starts, `_youtubePage` is brought back to the foreground (`BringToFrontAsync`) so it isn't throttled — the tab capture keeps working regardless of z-order. `EnsureSelfDeafenedAsync` polls for the deafen button (`DeafenMaxProbes`) since the voice panel renders after joining, and clicks it only when its `aria-label` starts with `Ativar`/`Deafen` (activate state), logging `Auto-surdo: clicked:<label>` / `already:<label>` / not-found for diagnosis.
